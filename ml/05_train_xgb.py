...
EXPERIMENT = "/Shared/wanderbricks_forecast"
...
def train_and_register(df: pd.DataFrame, test_size: float, n_estimators: int,
    ...
    with mlflow.start_run(run_name="xgb_weather"):
        ...
        # UC: no stages — point the Production alias at this new version
        MlflowClient().set_registered_model_alias(
            f"{catalog}.{schema}.{MODEL_NAME}", "Production", mv.version)
        print(f"registered {catalog}.{schema}.{MODEL_NAME} v{mv.version} -> @Production")
